...
read_from_path <-
  function (path) {
    tryCatch({
      ...
      cli::cli_alert_info(
        cli::col_blue("Reading Raw Data From Path: {path}")
      )
      envir <- environment()
      ...
      cli::cli_alert_success(
        cli::col_green("Successfully Read Data")
      )
      return(df)
    }, error = function(e) {
...
#' @export
data_split <-
  function(DF, prop, resp) {

    tryCatch({
      cli::cli_alert_info(
        cli::col_blue("Split Data on Proportion : {prop}")
      )

      splt <- withr::with_seed(
        seed = 1987,
        rsample::initial_split(
          data = DF, prop = prop, strata = dplyr::all_of(resp)
        )
      )

      cli::cli_alert_success(
        cli::col_green("Successfully Splitted Data")
      )
...
data_recipe <-
  function(DF, features) {
    ...
    tryCatch({
      ...
      cli::cli_alert_success(
        cli::col_green("Successfully Created Recipe")
      )
